...
int main(int argc, char **argv)
{
	...
	connect_to_db(dsn);
	printf("Dropping and creating database contrib_regression_linux...\n");
	run_statement("DROP DATABASE IF EXISTS contrib_regression_linux");
	run_statement("CREATE DATABASE contrib_regression_linux");

	snprintf(dsn, sizeof(dsn), "DSN=%s;Database=contrib_regression_linux", get_test_dsn());
	connect_to_db(dsn);
